...
from discord import ActionRow, ButtonStyle, user, app_commands
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if view.current_player == view.X:
            if interaction.user.id != player1:
                ...
                content = f"It is now <@{player2}>(O's) turn"

        else:
            if interaction.user.id != player2:
                await interaction.response.send_message("Its not your Turn!", ephemeral=True)
            else:
                ...
                content = f"It is now <@{player1}>(X's) turn"

        winner = view.check_board_winner()
        if winner is not None:
            if winner == view.X:
                content = f'<@{player1}>(X) won!'
            elif winner == view.O:
                content = f'<@{player2}>(O) won!'
            else:
                content = "It's a tie!"
